...
export function ChatView({ token }: ChatViewProps) {
  ...
  const fetchConversations = useCallback(async () => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || "http://192.168.1.134:3000"}/api/conversations`,
        {
          headers: {
  ...
  const fetchMessages = useCallback(
    async (conversationId: string) => {
      ...
      try {
        const response = await fetch(
          `${import.meta.env.VITE_API_URL || "http://192.168.1.134:3000"}/api/conversations/${conversationId}/messages`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
            },
  ...
  const handleSend = async () => {
    ...
    try {
      // Try REST API first
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || "http://192.168.1.134:3000"}/api/conversations/${selectedConversation.id}/messages`,
        {
          method: "POST",
          headers: {
